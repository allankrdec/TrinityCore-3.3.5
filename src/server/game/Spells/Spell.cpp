...
Spell::Spell(WorldObject *caster, SpellInfo const *info, TriggerCastFlags triggerFlags, ObjectGuid originalCasterGUID) : m_spellInfo(sSpellMgr->GetSpellForDifficultyFromSpell(info, caster)),
                                                                                                                         m_caster((info->HasAttribute(SPELL_ATTR6_CAST_BY_CHARMER) && caster->GetCharmerOrOwner()) ? caster->GetCharmerOrOwner() : caster), m_spellValue(new SpellValue(m_spellInfo)), _spellEvent(nullptr)
{
    ...
    m_canReflect = caster->IsUnit() && m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MAGIC && !m_spellInfo->HasAttribute(SPELL_ATTR0_ABILITY) && !m_spellInfo->HasAttribute(SPELL_ATTR1_CANT_BE_REFLECTED) && !m_spellInfo->HasAttribute(SPELL_ATTR0_UNAFFECTED_BY_INVULNERABILITY) && !m_spellInfo->IsPassive();
...
void Spell::InitExplicitTargets(SpellCastTargets const &targets)
{
    ...
    if (WorldObject *target = m_targets.GetObjectTarget())
    {
        ...
        if ((target->ToUnit() && !(neededTargets & (TARGET_FLAG_UNIT_MASK | TARGET_FLAG_CORPSE_MASK))) || (target->ToGameObject() && !(neededTargets & TARGET_FLAG_GAMEOBJECT_MASK)) || (target->ToCorpse() && !(neededTargets & TARGET_FLAG_CORPSE_MASK)))
...
void Spell::SelectImplicitChainTargets(SpellEffectInfo const &spellEffectInfo, SpellImplicitTargetInfo const &targetType, WorldObject *target, uint32 effMask)
{
    ...
    if (maxTargets > 1)
    {
        ...
        std::list<WorldObject *> targets;
        SearchChainTargets(targets, maxTargets - 1, target, targetType.GetObjectType(), targetType.GetCheckType(), spellEffectInfo.ImplicitTargetConditions, targetType.GetTarget() == TARGET_UNIT_TARGET_CHAINHEAL_ALLY);
...
void Spell::prepareDataForTriggerSystem()
{
    ...
    default:
        if (m_spellInfo->EquippedItemClass == ITEM_CLASS_WEAPON &&
            m_spellInfo->EquippedItemSubClassMask & (1 << ITEM_SUBCLASS_WEAPON_WAND) && m_spellInfo->HasAttribute(SPELL_ATTR2_AUTOREPEAT_FLAG)) // Wands auto attack
        {
            m_procAttacker = PROC_FLAG_DONE_RANGED_AUTO_ATTACK;
            m_procVictim = PROC_FLAG_TAKEN_RANGED_AUTO_ATTACK;
        }
...
void Spell::AddUnitTarget(Unit *target, uint32 effectMask, bool checkIfValid /*= true*/, bool implicit /*= true*/, Position const *losPosition /*= nullptr*/)
{
    ...
    auto ihit = std::find_if(std::begin(m_UniqueTargetInfo), std::end(m_UniqueTargetInfo), [targetGUID](TargetInfo const &target)
                             { return target.TargetGUID == targetGUID; });
...
void Spell::AddGOTarget(GameObject *go, uint32 effectMask)
{
    ...
    // Lookup target in already in list
    auto ihit = std::find_if(std::begin(m_UniqueGOTargetInfo), std::end(m_UniqueGOTargetInfo), [targetGUID](GOTargetInfo const &target)
                             { return target.TargetGUID == targetGUID; });
...
void Spell::AddItemTarget(Item *item, uint32 effectMask)
{
    ...
    // Lookup target in already in list
    auto ihit = std::find_if(std::begin(m_UniqueItemInfo), std::end(m_UniqueItemInfo), [item](ItemTargetInfo const &target)
                             { return target.TargetItem == item; });
...
SpellMissInfo Spell::PreprocessSpellHit(Unit *unit, bool scaleAura, TargetInfo &hitInfo)
{
    ...
    if (Aura::BuildEffectMaskForOwner(m_spellInfo, MAX_EFFECT_MASK, unit))
    {
        ...
        if (!hitInfo.Positive && !unit->ApplyDiminishingToDuration(hitInfo.AuraSpellInfo, triggered, hitInfo.AuraDuration, origCaster, diminishLevel))
            if (std::all_of(std::begin(hitInfo.AuraSpellInfo->GetEffects()), std::end(hitInfo.AuraSpellInfo->GetEffects()), [](SpellEffectInfo const &effInfo)
                            { return !effInfo.IsEffect() || effInfo.Effect == SPELL_EFFECT_APPLY_AURA; }))
                return SPELL_MISS_IMMUNE;
    }
...
void Spell::SendSpellGo()
{
    ...
    if ((m_caster->GetTypeId() == TYPEID_PLAYER) && (m_caster->ToPlayer()->GetClass() == CLASS_DEATH_KNIGHT) && m_spellInfo->RuneCostID && m_spellInfo->PowerType == POWER_RUNE && !(_triggeredCastFlags & TRIGGERED_IGNORE_POWER_AND_REAGENT_COST))
    {
        castFlags |= CAST_FLAG_NO_GCD;    // not needed, but Blizzard sends it
...
SpellCastResult Spell::CheckCast(bool strict, uint32 *param1 /*= nullptr*/, uint32 *param2 /*= nullptr*/)
{
    ...
    for (SpellEffectInfo const &spellEffectInfo : m_spellInfo->GetEffects())
    {
        ...
        case SPELL_AURA_AOE_CHARM:
        {
            ...
            if (spellEffectInfo.ApplyAuraName == SPELL_AURA_MOD_CHARM || spellEffectInfo.ApplyAuraName == SPELL_AURA_MOD_POSSESS)
            {
                if (!m_spellInfo->HasAttribute(SPELL_ATTR1_DISMISS_PET) && !unitCaster->GetPetGUID().IsEmpty())
                    return SPELL_FAILED_ALREADY_HAVE_SUMMON;
        ...
        case SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED:
        {
            ...
            if (m_originalCaster && m_originalCaster->GetTypeId() == TYPEID_PLAYER && m_originalCaster->IsAlive())
            {
                ...
                if (AreaTableEntry const *area = sAreaTableStore.LookupEntry(m_originalCaster->GetAreaId()))
                    if ((area->Flags & AREA_FLAG_NO_FLY_ZONE || (Bf && !Bf->CanFlyIn())) && m_originalCaster->GetMapId() != 0 // libera voo em azeroth
...
SpellCastResult Spell::CheckItems(uint32 *param1 /*= nullptr*/, uint32 *param2 /*= nullptr*/) const
{
    ...
    for (SpellEffectInfo const &spellEffectInfo : m_spellInfo->GetEffects())
    {
        ...
        case SPELL_EFFECT_ENCHANT_ITEM:
            if (spellEffectInfo.ItemType && m_targets.GetItemTarget() && (m_targets.GetItemTarget()->IsWeaponVellum() || m_targets.GetItemTarget()->IsArmorVellum()))
            {
        ...
        case SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC:
        {
            ...
            for (uint8 e = 0; e < MAX_ITEM_PROTO_SPELLS; ++e)
            {
                ...
                if (proto->Spells[e].SpellId > 0 && (proto->Spells[e].SpellTrigger == ITEM_SPELLTRIGGER_ON_USE ||
